...
import { X, ExternalLink, ThumbsUp, ThumbsDown, MapPin } from 'lucide-react';
...
const BrowserExtension = ({ onClose }: BrowserExtensionProps) => {
  ...
  // Function to extract date from article HTML
  const extractDate = (article) => {
    const dateElement = article.querySelector('[aria-label*="il y a"]') || 
                        article.querySelector('[aria-label="Aujourd\'hui"]');
    
    if (dateElement) {
      return dateElement.textContent.trim();
    }
    
    // Look for text patterns that might indicate dates
    const textElements = article.querySelectorAll('p.text-caption.text-neutral');
    for (const element of textElements) {
      const text = element.textContent.trim();
      if (text.includes('il y a') || text === 'Aujourd\'hui') {
        return text;
      }
    }
  ...
  const extractLocation = (article) => {
    ...
    for (const selector of selectors) {
      ...
      if (element) {
        ...
        if (text && !text.startsWith("il y a") && text !== "Aujourd'hui") {
          return text;
        }
      }
    }
            ...
            {products.length > 0 ? (
              <div className="space-y-4">
                ...
                {products.map((product) => {
                  ...
                  // Date extraite ou placeholder
                  const date = htmlData?.date || "Récemment";
                  ...
                  return (
                    ...
                    >
                      <div className="relative aspect-w-4 aspect-h-3 bg-gray-100">
                        {/* Date badge on top of image with semi-transparent background */}
                        <div className="absolute top-2 left-2 bg-black bg-opacity-60 text-white px-2 py-1 rounded-md text-xs font-medium shadow-sm">
                          {date}
                        </div>
                        <img 
                          src={product.image} 
